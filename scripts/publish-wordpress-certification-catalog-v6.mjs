...
const css = `<style id="dtf-courses-v6-style">.dc6{--ink:#17291d;--muted:#5b6b61;--deep:#082419;--green:#237345;--paper:#fff;--soft:#f3f7f3;--line:#d7e3d8;--gold:#c7a14b;background:linear-gradient(180deg,#faf8f1,#edf5ef);color:var(--ink);padding:30px 0 76px}.dc6 *{box-sizing:border-box}.dc6-wrap{width:min(1240px,calc(100% - 28px));margin:auto}.dc6-hero{display:grid;grid-template-columns:1.25fr .75fr;gap:24px;padding:clamp(28px,5vw,58px);border-radius:24px;background:linear-gradient(135deg,var(--deep),#18452c);color:#fff}.dc6-k{font-size:.75rem;font-weight:950;letter-spacing:.12em;text-transform:uppercase;color:#b6e3bd}.dc6 h1{font-size:clamp(2.4rem,5.8vw,5rem);letter-spacing:-.05em;line-height:.98;margin:.18em 0}.dc6 h2{font-size:clamp(1.7rem,3vw,2.45rem)}.dc6 p,.dc6 li{line-height:1.65}.dc6-hero p{color:#dfebe2}.dc6-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:20px}.dc6-btn{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:10px 15px;border-radius:10px;background:#2f8650;color:#fff!important;font-weight:850;text-decoration:none!important}.dc6-btn.secondary{background:transparent;border:1px solid rgba(255,255,255,.3)}.dc6-guide,.dc6-card,.dc6-course,.dc6-surface{border:1px solid var(--line);border-radius:16px;background:#fff}.dc6-guide{padding:19px;background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.14)}.dc6-guide p{color:#dfeae2}.dc6-section{margin-top:36px}.dc6-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}.dc6-card{padding:19px}.dc6-card h3{margin:10px 0 7px}.dc6-type{font-size:.76rem;font-weight:900;text-transform:uppercase;letter-spacing:.06em;color:#56705e}.dc6-status{display:inline-flex;padding:6px 9px;border-radius:999px;background:#edf4ee;color:#285d39;font-size:.72rem;font-weight:900;text-transform:uppercase}.dc6-course-list{display:grid;gap:13px}.dc6-course{display:grid;grid-template-columns:170px 1fr;overflow:hidden}.dc6-rail{padding:18px;background:#103220;color:#fff}.dc6-rail strong{display:block;margin-top:10px}.dc6-body{padding:22px}.dc6-body h3{margin:0;font-size:1.55rem}.dc6-body p{color:var(--muted)}.dc6-metrics{display:flex;flex-wrap:wrap;gap:7px;margin:14px 0}.dc6-metric{padding:9px 11px;border-radius:10px;background:var(--soft);font-size:.82rem}.dc6-metric strong{color:#185f36;margin-right:4px}.dc6-boundary{margin-top:18px;padding:15px 17px;border-left:5px solid var(--gold);border-radius:10px;background:#fff7df;color:#574c2c}.dc6-surfaces{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.dc6-surface{padding:18px}.dc6 :focus-visible{outline:3px solid #1763ae;outline-offset:3px}@media(max-width:850px){.dc6-hero,.dc6-grid,.dc6-surfaces{grid-template-columns:1fr}.dc6-course{grid-template-columns:140px 1fr}}@media(max-width:620px){.dc6-wrap{width:min(100% - 18px,1240px)}.dc6-hero{grid-template-columns:1fr;padding:23px 17px;border-radius:16px}.dc6-course{grid-template-columns:1fr}.dc6-actions{display:grid}.dc6-btn{width:100%}}</style>`;
const actions = (data.primaryActions || []).map(a => `<a class="dc6-btn${a.style === 'secondary' ? ' secondary' : ''}" href="${esc(a.href)}">${esc(a.label)}</a>`).join('');
const credentialSections = data.credentialSections.map(section => `<section class="dc6-section" data-credential-section="${esc(section.id)}"><h2>${esc(section.title)}</h2><p>${esc(section.description)}</p><div class="dc6-grid">${(section.offerings || []).map(item => `<article class="dc6-card" data-credential-offering="${esc(item.title)}" data-issuance-available="${item.issuanceAvailable ? 'true' : 'false'}"><span class="dc6-type">${esc(item.type)}${item.plannedCourses ? ` · ${esc(item.plannedCourses)}-course pathway` : ''}</span> <span class="dc6-status">${esc(item.statusLabel)}</span><h3>${esc(item.title)}</h3><p>${esc(item.summary)}</p>${item.availableCourse ? `<p><a href="${esc(item.availableCourse.href)}"><strong>${esc(item.availableCourse.label)} →</strong></a></p>` : ''}</article>`).join('')}</div></section>`).join('');
const courses = data.courses.map(course => `<article class="dc6-course" data-course-id="${esc(course.id)}" data-course-release="available"><aside class="dc6-rail"><span class="dc6-status">${esc(course.statusLabel || course.status)}</span><strong>${esc(course.pathLabel || '')}</strong></aside><div class="dc6-body"><h3>Course ${esc(course.number)} — ${esc(course.title)}</h3><p>${esc(course.summary)}</p><div class="dc6-metrics">${(course.metrics || []).map(m => `<span class="dc6-metric"><strong>${esc(m.value)}</strong>${esc(m.label)}</span>`).join('')}</div><a class="dc6-btn" data-course-open="true" href="${esc(course.href)}">Open Course ${esc(course.number)} →</a></div></article>`).join('');
...
const content = `${css}<main class="dc6" data-dtf-courses-catalog="v6"><div class="dc6-wrap"><section class="dc6-hero"><div><span class="dc6-k">${esc(data.eyebrow)}</span><h1>${esc(data.title)}</h1><p>${esc(data.intro)}</p><div class="dc6-actions">${actions}</div></div><aside class="dc6-guide"><strong>Training availability and credential issuance are separate.</strong><p>All Technician I and II academic courses are open for study and learning assessment. No professional credential shown here is currently available for issuance until its validation and governance gates are complete.</p></aside></section><section class="dc6-section"><h2>Certificates & professional certifications</h2>${credentialSections}<div class="dc6-boundary"><strong>Credential boundary:</strong> course publication does not authorize certification issuance.</div></section><section class="dc6-section"><h2>${esc(data.coursesSection?.title || 'Technician I & II courses')}</h2><p>${esc(data.coursesSection?.intro || '')}</p><div class="dc6-course-list">${courses}</div></section><section class="dc6-section"><h2>Know where you are learning</h2><div class="dc6-surfaces">${surfaces}</div><div class="dc6-boundary"><strong>Assessment boundary:</strong> ${esc(data.assessmentNote || 'Course tests are learning assessments and do not issue a professional credential.')}</div></section></div></main>`;
...
if (validateOnly) { console.log(JSON.stringify({ result: 'success', version: 6, openCourses: open.map(x => x.id) }, null, 2)); process.exit(0); }
...
console.log(JSON.stringify({ result: 'success', version: 6, pageId: updated.id, openCourses: open.map(x => ({ id: x.id, href: x.href })) }, null, 2));
