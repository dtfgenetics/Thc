...
if (!source.includes(BASE_VERSION)) {
  throw new Error(`Seed Man combat runtime must contain ${BASE_VERSION} before v3 enhancement.`);
}

const visualMarker = `  const ENEMY_VISUAL_VERSION = '${BASE_VERSION}';`;
const drawEnemyMarker = '  function drawEnemy(enemy) {';
if (!source.includes(visualMarker) || !source.includes(drawEnemyMarker)) {
  throw new Error('Seed Man enemy visual v2 runtime is missing expected v3 patch anchors.');
}
