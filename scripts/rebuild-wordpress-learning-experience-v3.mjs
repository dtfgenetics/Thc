...
function buildLearn() {
  ...
  <section class="hero"><div class="wrap hero-grid"><div><p class="kicker">Teaching Healthy Cultivation</p><h1>Learn the plant as one connected system.</h1><p class="lede">Move from anatomy and plant function into environment, roots, nutrition, crop management, diagnostics, harvest, and breeding. Every subject is built to connect observation with measurement, evidence, and a clear next question.</p><div class="actions">${btn('/learn/start-here/', 'Start here', true)}${btn('/learn/search/', 'Search education', false)}${btn('/learn/infographics/', 'Browse visuals', false)}</div></div><div class="hero-media">${img(topicImages['plant-biology'] || fallbackHero, 'Cannabis plant biology educational reference', { ratio: '1/1', eager: true })}</div></div></section>
  ...
  <section class="section soft"><div class="wrap"><div class="heading"><div><p class="eyebrow">Reference desk</p><h2>Keep the core references beside the lesson.</h2></div><p>Use the Atlas and measurement references while reading so anatomy, chemistry, root-zone numbers, and environment stay connected to the concept you are studying.</p></div><div class="reference-desk"><a href="/atlas/" target="_blank" rel="noopener"><b>Plant Atlas</b><span>Interactive anatomy, structures, systems, and diagnostics.</span></a><a href="/terpene-atlas/" target="_blank" rel="noopener"><b>Terpene Atlas</b><span>Compound families, measured samples, chemistry, and evidence.</span></a><a href="/ph-meter/" target="_blank" rel="noopener"><b>pH Reference</b><span>Interpret calibrated pH readings and root-zone context.</span></a><a href="/tds-meter/" target="_blank" rel="noopener"><b>TDS / EC</b><span>Conductivity and 500/700-scale conversion reference.</span></a><a href="/vpd-chart/" target="_blank" rel="noopener"><b>VPD Chart</b><span>Air temperature, RH, leaf temperature, and vapor-pressure context.</span></a></div></div></section>
...
const topicToolLinks = {
  'plant-biology': [['/atlas/','Plant Atlas','Inspect anatomy, systems, and structures beside the lesson.']],
  'genetics-breeding': [['/atlas/genetics-phenotype/','Genetics Atlas','Connect inheritance and phenotype concepts to plant biology.'],['/growlens/','GrowLens','Keep selection observations and repeatable records.']],
  'lifecycle-propagation': [['/atlas/seed-germination/','Seed & Germination Atlas','Review anatomy and early-development structures.'],['/growlens/','GrowLens','Record dates, stage changes, rooting, transplant, and acclimation.']],
  'environment-vpd': [['/vpd-chart/','VPD Chart','Calculate leaf-VPD context from temperature, RH, and leaf offset.'],['/growlens/','GrowLens','Track environmental trends and events.']],
  'lighting': [['/growlens/','GrowLens','Record PPFD, DLI, fixture state, and canopy observations.']],
  'water-ph-ec': [['/ph-meter/','pH Reference','Interpret calibrated pH readings in context.'],['/tds-meter/','TDS / EC Meter','Convert and preserve EC/PPM scale context.'],['/growlens/','GrowLens','Track irrigation, runoff, dryback, and meter readings.']],
  'nutrition-media': [['/ph-meter/','pH Reference','Keep pH chemistry separate from nutrient diagnosis.'],['/tds-meter/','TDS / EC Meter','Track conductivity without treating EC as nutrient identity.'],['/thc-grow-doc/','Grow Doc','Compare nutrient-like symptoms against root and environment look-alikes.']],
  'plant-health-ipm': [['/thc-grow-doc/','Grow Doc','Build an evidence-based differential before acting.'],['/atlas/diagnostics/','Diagnostic Atlas','Compare anatomy and symptom location.']],
  'training-canopy': [['/atlas/nodes-branching/','Branching Atlas','Review nodes, internodes, meristems, and branch structure.'],['/growlens/','GrowLens','Document before/after canopy measurements.']],
  'harvest-postharvest': [['/atlas/trichomes-resin/','Trichome Atlas','Inspect trichome biology without treating color alone as a harvest rule.'],['/growlens/','GrowLens','Track harvest batches, drying conditions, and observations.']],
  'outdoor': [['/growlens/','GrowLens','Record weather, irrigation, stage, and outdoor observations.']],
  'research-measurement': [['/growlens/','GrowLens','Preserve measurements, methods, timestamps, and outcomes.'],['/tools/','Reference Tools','Open pH, EC/TDS, VPD, and Atlas references.']],
  'general-reference': [['/tools/','Reference Tools','Open the core measurement and Atlas references.']]
};
...
function buildTopic(topic) {
  ...
  const sectionId = (heading, index) => `topic-section-${String(index + 1).padStart(2, '0')}-${String(heading || '').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,42)}`;
  const lessonCards = (topic.sections || []).map((section, index) => {
    const id = sectionId(section.heading, index);
    return `<details class="lesson" id="${esc(id)}" data-progressive-disclosure="true"><summary><span class="pill">${String(index + 1).padStart(2, '0')}</span><span class="lesson-title">${esc(section.heading)}</span></summary><div class="lesson-body">${(section.paragraphs || []).map(p => `<p>${esc(p)}</p>`).join('')}${section.checkpoints?.length ? `<ul class="checks">${section.checkpoints.map(item => `<li>${esc(item)}</li>`).join('')}</ul>` : ''}</div></details>`;
  }).join('');
  const toc = (topic.sections || []).map((section,index)=>`<a href="#${esc(sectionId(section.heading,index))}">${String(index+1).padStart(2,'0')} · ${esc(section.heading)}</a>`).join('');
  ...
  const guide = topic.referenceGuide || {};
  const guideBlock = (label, items) => Array.isArray(items) && items.length
    ? `<article><h3>${esc(label)}</h3><ul>${items.map(item => `<li>${esc(item)}</li>`).join('')}</ul></article>`
    : '';
  const fieldGuideMarkup = [
    guideBlock('Observe', guide.observe),
    guideBlock('Measure', guide.measure),
    guideBlock('Do not infer', guide.avoid)
  ].join('');
  ...
  <section class="section"><div class="wrap"><div class="heading"><div><p class="eyebrow">Use the subject</p><h2>Read with an observation framework.</h2></div><p>The same four questions keep the literature practical without pretending one symptom, number, or photograph proves a cause.</p></div><div class="quick-reference"><article><strong>Observe</strong><p>Describe the plant or system before naming a cause.</p></article><article><strong>Measure</strong><p>Use the measurement that actually tests the question.</p></article><article><strong>Compare</strong><p>Keep plausible alternatives until evidence separates them.</p></article><article><strong>Record</strong><p>Preserve method, context, intervention, and response over time.</p></article></div>${fieldGuideMarkup ? `<div class="field-guide">${fieldGuideMarkup}</div>` : ''}<div class="reference-desk">${tools}</div></div></section>

  <section class="section soft"><div class="wrap"><div class="heading"><div><p class="eyebrow">Core literature</p><h2>Build the model before making the decision.</h2></div><p>Use the section navigator to move through the topic as a reference instead of scanning a wall of equal-weight cards.</p></div><div class="topic-shell"><nav class="topic-toc" aria-label="${esc(topic.title)} section navigation"><strong>On this page</strong>${toc}</nav><div class="lesson-grid">${lessonCards}</div></div></div></section>

  <section class="section"><div class="wrap"><div class="heading"><div><p class="eyebrow">Visual references</p><h2>Use images to clarify structure, pattern, and measurement.</h2></div><p>Only approved role-specific visuals appear here. If a visual has not passed subject and responsive review, the literature remains available without filler imagery.</p></div>${related.length ? `<div class="visual-grid">${related.map(item => `<figure class="visual"><a href="/learn/infographics/">${img(item, `${topic.title} visual reference`, { ratio: '4/3' })}</a><figcaption>${esc(plain(rendered(item.title)) || topic.title)}</figcaption></figure>`).join('')}</div>` : `<div class="ref-card"><span class="pill">Visual production</span><h3>Role-specific references are being rebuilt for this subject.</h3><p>The old generic infographic family is intentionally not used as a placeholder. Open the Visual Reference Library to see the production slots defined for this subject.</p>${textLink('/learn/infographics/','Open visual reference library')}</div>`}<div class="actions light-actions">${btn('/learn/infographics/', 'Open visual reference library', true)}</div></div></section>

  ${references ? `<section class="section soft"><div class="wrap"><div class="heading"><div><p class="eyebrow">Sources & further reading</p><h2>Follow the framework behind the lesson.</h2></div><p>References support the scientific model and measurement approach; they are not used as a substitute for crop-specific measurements or local legal requirements.</p></div><div class="ref-grid">${references}</div></div></section>` : ''}
