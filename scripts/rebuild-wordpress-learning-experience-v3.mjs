...
function buildTopic(topic) {
  ...
  const lessonCards = (topic.sections || []).map((section, index) => {
    ...
    return `<article class="lesson" id="${esc(id)}"><span class="pill">${String(index + 1).padStart(2, '0')}</span><h2>${esc(section.heading)}</h2>${(section.paragraphs || []).map(p => `<p>${esc(p)}</p>`).join('')}${section.checkpoints?.length ? `<ul class="checks">${section.checkpoints.map(item => `<li>${esc(item)}</li>`).join('')}</ul>` : ''}</article>`;
  }).join('');
