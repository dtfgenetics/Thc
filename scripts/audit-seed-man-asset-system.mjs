...
const levels = readJson('games', 'seed-man-platformer', 'data', 'levels-20-v1.json');
const recipes = readJson('games', 'seed-man-platformer', 'data', 'authored-level-recipes-v1.json');
const worlds = readJson('games', 'seed-man-platformer', 'data', 'world-gameplay-v1.json');
const powerups = readJson('games', 'seed-man-platformer', 'data', 'powerup-catalog-v1.json');
...
const hazardRuntime = readText('games', 'seed-man-platformer', 'src', 'systems', 'hazard-system.mjs');
const encounterRuntime = readText('games', 'seed-man-platformer', 'src', 'systems', 'encounter-director.mjs');
...
check('sm001-approved-complete',Array.isArray(sm001.assets) && sm001.assets.length === 10 && sm001.assets.every((asset) => asset.state === 'APPROVED'),'critical','All SM-001 canonical reference masters must be approved before animation production is unblocked.',{ states: sm001.assets?.map((asset) => ({ assetId: asset.assetId, state: asset.state })) ?? [] });
...
check('all-levels-authored',Number(game.campaign?.generatedLevels || 0) === 0 && Number(game.campaign?.authoredLevels || 0) === Number(game.campaign?.levels || 0),'high','All campaign levels must be authored before visual production is considered complete.',{ authored: game.campaign?.authoredLevels, generated: game.campaign?.generatedLevels, total: game.campaign?.levels });
...
const summary = {
  ...
  strict,
  assets: {observedAssetFiles:allAssetFiles.length,approvedBinaryFiles:approvedBinaryFiles.length,authoredWorldArtFiles:worldArtFiles.length,plannedProductionBatches:audit.plannedProduction?.batchCount,nominalProductionDeliverableGroups:audit.plannedProduction?.nominalDeliverableGroups},
  campaign: {levels:game.campaign?.levels,authoredLevels:game.campaign?.authoredLevels,generatedLevels:game.campaign?.generatedLevels,explicitLayouts,authoredRecipes:recipeCount},
