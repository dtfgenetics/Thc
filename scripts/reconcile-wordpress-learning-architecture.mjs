    ...
    try {
      const response = await fetch(`${site}${path}`, {
        ...
        headers: {
          ...
          'User-Agent': 'DTF-Learning-Architecture-Reconciler/1.0',
          ...(options.headers || {})
        }
      });
...
function verify(content, label) {
  ...
  must((content.match(new RegExp(learningHubMarker, 'g')) || []).length === 1, `${label}: Learning Hub marker must occur exactly once.`);
}
