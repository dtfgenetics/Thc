...
.t1c{--ink:#183223;--muted:#5a6b60;--deep:#0b281a;--green:#227244;--soft:#f4f8f4;--line:#d8e4da;--gold:#c9a74f;background:linear-gradient(180deg,#fafbf7,#f1f6f1);color:var(--ink);padding:32px 0 72px}.t1c *{box-sizing:border-box}.t1c-wrap{width:min(1120px,calc(100% - 28px));margin:auto}.t1c-hero{padding:clamp(26px,5vw,54px);border-radius:22px;background:linear-gradient(135deg,var(--deep),#1b4a2f);color:#fff}.t1c-k{font-size:.75rem;font-weight:950;letter-spacing:.12em;text-transform:uppercase;color:#b8e3be}.t1c h1{font-size:clamp(2.2rem,5vw,4.6rem);line-height:1;letter-spacing:-.045em;margin:.2em 0}.t1c h2{font-size:clamp(1.5rem,3vw,2.25rem);line-height:1.12}.t1c p,.t1c li{line-height:1.7}.t1c-hero p{color:#dfece2;max-width:820px}.t1c-nav{display:flex;flex-wrap:wrap;gap:8px;margin:16px 0}.t1c-nav a,.t1c-btn{display:inline-flex;align-items:center;min-height:44px;padding:9px 13px;border:1px solid var(--line);border-radius:10px;background:#fff;color:#175f36!important;font-weight:850;text-decoration:none!important}.t1c-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.t1c-card,.t1c-panel,.t1c-q{background:#fff;border:1px solid var(--line);border-radius:16px;padding:20px}.t1c-card h3,.t1c-panel h2{margin-top:0}.t1c-meta{display:flex;flex-wrap:wrap;gap:8px;margin:16px 0}.t1c-pill{padding:6px 9px;border-radius:999px;background:#eaf3eb;color:#285d3a;font-size:.78rem;font-weight:850}.t1c-boundary{margin:18px 0;padding:15px 17px;border-left:5px solid var(--gold);background:#fff7df;border-radius:10px;color:#554b2c}.t1c-callout{margin:18px 0;padding:16px;border:1px solid #d5c896;background:#fff9e8;border-radius:12px}.t1c-section{margin-top:28px}.t1c-vocab{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.t1c-vocab div{padding:14px;background:var(--soft);border-radius:12px}.t1c-step{padding:14px 16px;border-left:4px solid #4a9a61;background:#f7faf7;margin:10px 0}.t1c-scenario{padding:18px;background:#f8f5e9;border:1px solid #e6dcc1;border-radius:14px}.t1c-resource{display:flex;flex-direction:column;gap:10px}.t1c-resource a{align-self:flex-start}.t1c-figure img{display:block;width:100%;max-height:760px;object-fit:contain;background:#fff;border:1px solid var(--line)}.t1c-docfields{display:grid;gap:8px}.t1c-docfield{padding:10px 12px;background:var(--soft);border-radius:9px}.t1c details{background:#fff;border:1px solid var(--line);border-radius:12px;padding:0 14px}.t1c summary{cursor:pointer;font-weight:900;padding:12px 0}.t1c-q{margin:12px 0}.t1c-q ol{padding-left:24px}.t1c-table{overflow:auto}.t1c table{width:100%;border-collapse:collapse;min-width:620px}.t1c th,.t1c td{border:1px solid var(--line);padding:10px;text-align:left;vertical-align:top}.t1c th{background:var(--soft)}.t1c :focus-visible{outline:3px solid #155fbd;outline-offset:3px}@media(max-width:760px){.t1c-grid,.t1c-vocab{grid-template-columns:1fr}.t1c-hero{border-radius:16px;padding:24px 18px}.t1c-wrap{width:min(100% - 18px,1120px)}.t1c table{min-width:540px}}
@media(max-width:640px){
.t1c{padding:12px 0 calc(84px + env(safe-area-inset-bottom));font-size:16.5px}
.t1c-wrap{width:min(100% - 28px,1120px)}
.t1c-crumbs{margin:4px 0 12px;display:flex;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;padding-bottom:2px}
.t1c-crumbs::-webkit-scrollbar{display:none}
.t1c-crumbs a,.t1c-crumbs .t1c-pill{flex:0 0 auto;min-height:44px;white-space:nowrap}
.t1c-hero{padding:22px 18px 20px;border-radius:15px;box-shadow:none}
.t1c h1{font-size:clamp(2rem,9.8vw,3rem);line-height:.98;letter-spacing:-.035em;text-wrap:balance}
.t1c h2{font-size:clamp(1.35rem,6.5vw,1.75rem);line-height:1.12}
.t1c h3{font-size:1.08rem}
.t1c p,.t1c li{line-height:1.62}
.t1c-hero p{font-size:.98rem;line-height:1.58}
.t1c-meta{gap:6px;margin:13px 0 0;overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none}
.t1c-meta::-webkit-scrollbar{display:none}
.t1c-pill{flex:0 0 auto;font-size:.73rem}
.t1c-section{margin-top:22px}
.t1c-card,.t1c-panel,.t1c-q{padding:16px;border-radius:14px}
.t1c-q{margin:10px 0}
.t1c-q ol{padding-left:22px}
.t1c-btn{min-height:46px;padding:10px 14px}
.t1c-callout,.t1c-scenario{padding:15px}
.t1c-step{padding:13px 14px}
.t1c-table{margin-inline:-2px;padding-bottom:8px;overflow-x:auto;overscroll-behavior-inline:contain}
.t1c table{min-width:520px;font-size:.88rem}
.t1c th,.t1c td{padding:9px}
.t1c-figure{margin-left:0;margin-right:0}
.t1c-figure img{max-height:none;border-radius:12px}
.t1c-figure figcaption{padding-top:8px;color:var(--muted);font-size:.88rem;line-height:1.45}
.t1c-mobile-progress{display:grid;gap:7px;margin:0 0 12px;font-size:.75rem;font-weight:850;color:#496052}
.t1c-mobile-progress>div{height:5px;overflow:hidden;border-radius:999px;background:#dce8de}
.t1c-mobile-progress i{display:block;height:100%;border-radius:inherit;background:var(--green)}
.t1c-lesson-nav{position:fixed;z-index:40;left:0;right:0;bottom:0;margin:0;padding:9px 12px calc(9px + env(safe-area-inset-bottom));display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:8px;background:rgba(250,251,247,.96);border-top:1px solid var(--line);box-shadow:0 -8px 24px rgba(17,45,28,.08);backdrop-filter:blur(12px)}
.t1c-lesson-nav .t1c-btn{width:auto;min-width:0;padding:9px 11px;justify-content:center}
.t1c-lesson-nav .t1c-btn:first-of-type{grid-column:1}
.t1c-lesson-nav .t1c-btn:last-of-type{grid-column:3}
.t1c-lesson-count{grid-column:2;grid-row:1;font-size:.72rem;font-weight:900;color:var(--muted);white-space:nowrap}
.t1c-boundary{padding:13px 14px;font-size:.88rem}
}
@media(min-width:641px){.t1c-mobile-progress,.t1c-lesson-count{display:none}}
@media print{.t1c{background:#fff;padding:0}.t1c-nav{display:none}}
</style>`;
const shell = body => `${css}<main class="t1c" data-tech1-source-ref="${esc(sourceRef)}"><div class="t1c-wrap">${body}</div></main>`;
const crumbs = (course, title = '') => `<nav class="t1c-nav t1c-crumbs" aria-label="Learning Hub breadcrumb"><a href="/courses/">Courses</a><a href="${esc(config.program.route)}">${esc(config.program.title)}</a><a href="${esc(course.route)}">Course ${course.number}</a>${title ? `<span class="t1c-pill">${esc(title)}</span>` : ''}</nav>`;
...
function renderAssessment(course, assessment, label) {
  ...
  return shell(`${crumbs(course, label)}<header class="t1c-hero"><span class="t1c-k">Course ${course.number} · ${esc(label)}</span><h1>${esc(assessment.title)}</h1><p>Complete each item before opening the answer panel. Use missed rationales to return to the matching lesson.</p></header><div class="t1c-boundary"><strong>Formative learning check:</strong> this self-check is for study only and does not create a graded certification record.</div><section class="t1c-section">${questions}</section>`);
}
