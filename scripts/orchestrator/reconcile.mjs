...
export function classifyReconciliation({ job, branchExists = false, uniqueCommits = 0, openPr = null, mergedPr = null, now = new Date() }) {
  ...
  if (!job.lease) {
    if (branchExists && uniqueCommits > 0) return { action: 'RECOVER_BRANCH', reason: 'unique-work-without-lease', branch: job.branch || null }
  ...
  const disposition = recoveryDisposition({ branchExists, uniqueCommits, openPr: false })
  if (disposition === 'REQUEUE') return { action: 'REQUEUE', reason: 'expired-lease-no-unique-work' }
  if (disposition === 'RECOVER_BRANCH') return { action: 'RECOVER_BRANCH', reason: 'expired-lease-unique-work', branch: job.branch || null }
