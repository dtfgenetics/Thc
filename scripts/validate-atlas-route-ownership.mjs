...
const terpeneAtlas = learnSections.find((entry) => entry.route === '/terpene-atlas/');
...
assert.ok(terpeneAtlas, 'Learn navigation must expose /terpene-atlas/');
assert.match(terpeneAtlas.label, /Terpene Atlas/i, '/terpene-atlas/ must be labeled as the Terpene Atlas');
...
const terpeneOwnership = (navigation.informationArchitecture?.specialOwnership ?? [])
  .find((entry) => entry.route === '/terpene-atlas/');
assert.equal(terpeneOwnership?.root, 'learn', '/terpene-atlas/ must belong primarily to the Learn information-architecture root');
assert.match(terpeneOwnership?.note ?? '', /sample-backed|quantitative cultivar/i, 'Terpene Atlas ownership must preserve the sample-backed measurement rule');
...
const toolTerpeneAtlas = navigation.tools.find((entry) => entry.id === 'terpene-atlas');
...
assert.equal(toolTerpeneAtlas?.route, '/terpene-atlas/', 'public Terpene Atlas tool must point to /terpene-atlas/');
