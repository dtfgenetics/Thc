...
if(!['seed-man-v20-enemy-runtime-v1','seed-man-v20-enemy-runtime-v2'].includes(enemyRuntimeVersion))throw new Error(`missing-marker:v20-enemy-runtime.js:${enemyRuntimeVersion||'missing-version'}`);
if(!enemyRuntime.includes("['fire','electric','ice']"))throw new Error('missing-marker:v20-enemy-runtime.js:temporary-phenotype-order');
...
console.log(JSON.stringify({ok:true,levels:20,worlds:5,bosses:6,finalBoss:'blight-king',approvedArt:true,approvedArtManifest:art.id,combat:'v2',enemyRuntime:enemyRuntimeVersion,phenotypes:['plant','fire','electric','ice']}));
