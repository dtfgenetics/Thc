...
const headers = { Authorization: auth, Accept: 'application/json', 'User-Agent': 'DTFSeeds-Learning-Expansion-Publisher/1.0' };
...
function sourceContent(html) {
  ...
  return `${style}\n<!-- DTF-PUBLIC-LEARNING-EXPANSION-V1 -->\n${main}`;
}
...
for (const result of results) {
  ...
  for (let attempt = 1; attempt <= 8; attempt++) {
    const response = await fetch(`${result.url}?dtf_expansion=${Date.now()}-${attempt}`, {
      headers: { 'Cache-Control': 'no-cache, no-store, max-age=0', Pragma: 'no-cache', 'User-Agent': 'DTFSeeds-Learning-Expansion-Publisher/1.0' },
      redirect: 'follow', signal: AbortSignal.timeout(60_000)
    });
    html = await response.text();
    if (response.ok && html.includes('Teaching Healthy Cultivation') && html.includes('DTF-PUBLIC-LEARNING-EXPANSION-V1')) { ok = true; break; }
    await new Promise((resolve) => setTimeout(resolve, 3000));
  }
  if (!ok) throw new Error(`Visitor-facing verification failed for ${result.url}`);
  for (const forbidden of ['email@email.com', '+123456789', 'being rebuilt', 'Needed from owner', 'Mystery_Line_F1_Regular_DTF_Strain_Card', 'Rainbow_Bubblegum_F1_Regular_DTF_Strain_Card']) {
