...
function validateLocalPackage(d) {
  ...
  must(d.source?.repository === 'dtfgenetics/Thc-learning-courses-', 'Course 1 must source canonical curriculum from the THC learning-courses repository.');
  must(/^[0-9a-f]{40}$/i.test(d.source?.ref || ''), 'Course 1 site source must pin an exact 40-character Academy commit.');
  must(d.delivery?.consumerRepository === 'dtfgenetics/Thc', 'Course 1 site delivery must identify the DTFSeeds consumer repository.');
  must(d.delivery?.curriculumEditableHere === false, 'The site package must not declare curriculum editable in the consumer repository.');
...
async function fetchText(url) {
  ...
  for (let attempt=1; attempt<=6; attempt++) {
    try {
      const r = await fetch(url, { signal:AbortSignal.timeout(30000), headers:{'User-Agent':'DTF-Learning-Hub-Course1/2.1'} });
...
async function loadSources(){
  ...
  const authorizedItemCount=Number(release.publicScope.publicCourseItems);
  must(Number.isInteger(authorizedItemCount)&&authorizedItemCount>0,'Canonical public release has an invalid publicCourseItems value.');
  ...
  must(itemCount===authorizedItemCount,`Canonical release authorizes ${authorizedItemCount} public Course 1 items, but ${itemCount} were resolved from the pinned assessments.`);
  return {release,modules,docs,assessments,itemCount};
}
...
if(validateOnly){console.log(JSON.stringify({valid:true,id:local.id,course:local.course.id,modules:local.modules.length,learnerDocuments:local.learnerDocuments.length,sourceRepo,sourceRef},null,2));process.exit(0);}
...
async function wp(path,options={}){let last;for(let attempt=1;attempt<=8;attempt++){try{const r=await fetch(`${site}${path}`,{...options,redirect:'follow',signal:AbortSignal.timeout(60000),headers:{Authorization:auth,Accept:'application/json','User-Agent':'DTF-Learning-Hub-Course1/2.1',...(options.body?{'Content-Type':'application/json'}:{}),...(options.headers||{})}});const text=await r.text();let body=text;try{body=text?JSON.parse(text):null}catch{}if((r.status===429||r.status>=500)&&attempt<8){await sleep(attempt*1200);continue;}if(!r.ok)throw new Error(`${options.method||'GET'} ${path} failed (${r.status}): ${typeof body==='string'?body.slice(0,500):JSON.stringify(body).slice(0,500)}`);return body;}catch(error){last=error;if(attempt<8)await sleep(attempt*1200);}}throw last;}
...
for(let i=0;i<local.modules.length;i++){const a=sources.assessments[i];const p=await upsert({slug:`test-module-${i+1}`,title:`Module ${i+1} Learning Test`,parent:course.id,content:renderTest(a,`Course 1 · Module ${i+1} learning test`)});pages.push({type:'module-test',number:i+1,id:p.id});}
...
const report={generatedAt:new Date().toISOString(),sourceRepository:sourceRepo,sourceRef,sourceRelease:sources.release.id,sourceCourse:local.course.id,publicItems:sources.itemCount,pages,backupDir};await writeFile(join(backupDir,'report.json'),`${JSON.stringify(report,null,2)}\n`);console.log(JSON.stringify(report,null,2));
