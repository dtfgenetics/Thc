...
if (JSON.stringify(registry.lifecycle) !== JSON.stringify(expectedLifecycle)) {
  fail(`lifecycle mismatch: ${JSON.stringify(registry.lifecycle)}`);
}
...
if (JSON.stringify(registry.drive?.visualSubfolders) !== JSON.stringify(expectedSubfolders)) {
  fail('Drive visual subfolder template changed unexpectedly');
}
...
if (!wave1 || JSON.stringify(wave1.games) !== JSON.stringify(expectedWave1)) {
  fail('Wave 1 order must remain Seed Man, Who Took It, High Life, High IQ, Terpocalypse, Bud or Bluff');
}
...
for (const [field, expected] of Object.entries({
  suspectPortraitsComplete: 25,
  itemEvidenceCardsMissing: 18,
  boardFramesMissing: 3,
  selectionStateAssetsMissing: 4,
})) {
...
for (const [field, expected] of Object.entries({
  brandingMissing: 3,
  uiMissing: 8,
  weaponFramesMissing: 6,
  enemyStatesMissing: 9,
  pickupsMissing: 5,
  effectsMissing: 4,
})) {
...
for (const marker of [
  'Google Drive = human/source asset library',
  'GitHub = runtime asset library',
...
for (const marker of [
  '04 Games/<Game>/08 Visual Assets',
...
if (!process.exitCode) {
  console.log(`Game asset production registry valid: ${allWaveGames.length} games across ${waves.length} waves; ${expectedWave1.length} detailed Wave 1 mappings.`);
}
