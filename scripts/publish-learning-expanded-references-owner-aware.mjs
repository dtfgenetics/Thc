...
if (roots.length !== 1) throw new Error(`Expected exactly one published root Learn page, found ${roots.length}`);
const page = roots[0];
...
const required = ['data-dtf-layout="learn-v3"', 'data-dtf-learning-map="v4"', 'data-dtf-learning-expanded-reference="v1"', heading, ownershipPhrase, ...routes.map(([href]) => href)];
for (const marker of required) if (!content.includes(marker)) throw new Error(`Expanded Learn output is missing ${marker}`);
...
if (Number(storedPage?.id || 0) !== Number(page.id) || storedPage?.status !== 'publish' || missing.length) {
  throw new Error(`Stored expanded Learn owner verification failed: id=${storedPage?.id || 0} status=${storedPage?.status || 'missing'} missing=${missing.join(',')}`);
}
