...
async function request(path,options={}){
  ...
  for(let attempt=1;attempt<=6;attempt+=1){
    try{
      const response=await fetch(`${siteUrl}${path}`,{...options,redirect:'follow',signal:AbortSignal.timeout(60000),headers:{Authorization:auth,Accept:'application/json','Cache-Control':'no-cache, no-store, max-age=0',Pragma:'no-cache','User-Agent':'DTFSeeds-V3-Learn-Finalizer/1.1',...(options.body?{'Content-Type':'application/json'}:{}),...(options.headers||{})}});
...
async function getLearnOwnedByV3(){
  let lastRows=null;
  for(let attempt=1;attempt<=8;attempt+=1){
    const rows=await request(`/wp-json/wp/v2/pages?slug=learn&context=edit&per_page=10&_dtf_v3_finalize=${Date.now()}-${attempt}`);
    lastRows=rows;
    if(Array.isArray(rows)&&rows.length===1){
      const content=rendered(rows[0].content);
      if(content.includes('dtf-visual-polish-v3')) return {page:rows[0],content};
    }
    if(attempt<8) await sleep(attempt*1000);
  }
  if(!Array.isArray(lastRows)||lastRows.length!==1) throw new Error(`Expected exactly one Learn page; found ${Array.isArray(lastRows)?lastRows.length:'invalid'}.`);
  throw new Error('Learn is not currently owned by the V3 visual layer after repeated cache-busted reads; refusing to write.');
}
const owned=await getLearnOwnedByV3();
