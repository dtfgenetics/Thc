...
function buildSnippetCode(source) {
  ...
  // MU plugins load before normal plugins. If the canonical MU owner becomes
  // available later, this fallback stays installed but becomes inert, avoiding
  // duplicate title/meta hooks while preserving an automatic migration path.
...
// Create the reviewed replacement while the previous fallback still exists but
// is inactive only during the final handoff. If activation fails, restore the
// prior active state before surfacing the deployment failure.
