...
for(const required of ['three-world-v1.js','campaign-v20-runtime.js','campaign-ui-v20.js','world-mechanics-browser-v1.js','v20-enemy-runtime.js','combat-browser-v2.js','enemy-attacks-browser-v2.js','approved-art-core-v1.js','approved-art-runtime-v1.js','seed-man-production-art.js','player-state-v20.js'])if(!indexText.includes(required))throw new Error(`Seed Man public index missing v20 runtime: ${required}`);
...
for(const marker of ['seed-man-runtime-health-v21','seed-man-runtime-bootstrap-v20','player-state-v20.js','three-world-adapter-v1.js','world-mechanics-browser-v1.js','seed-man-three-public-v3'])if(!compatText.includes(marker))throw new Error(`Seed Man runtime health bridge missing marker: ${marker}`);
const worldMechanicsText=fs.readFileSync(path.join(root,'world-mechanics-browser-v1.js'),'utf8');
for(const marker of ['seed-man-world-mechanics-browser-v3','seed-man-world-mechanics-runtime-v1','moving-platforms','collapsing-platforms','conveyor-platforms','heat-updraft'])if(!worldMechanicsText.includes(marker))throw new Error(`Seed Man world mechanics runtime missing marker: ${marker}`);
const threeText=fs.readFileSync(path.join(root,'three-world-v1.js'),'utf8');
if(!threeText.includes('SeedManThreeWorld')||!threeText.includes('seed-man-three-public-v3')||!threeText.includes('seed-man-three-world-v2')||!threeText.includes('seed-man-three-dynamic-platforms-v1')||Buffer.byteLength(threeText)<250000)throw new Error('Generated Seed Man Three.js world bundle is missing, lacks dynamic-platform support, or is still a compatibility stub.');
...
add_action('rest_api_init', function () {
  ...
  register_rest_route($ns, '/publish', [
    ...
    'callback' => static function(WP_REST_Request $r) use ($token,$allowed,$retired,$purge,$remove) {
      ...
      if(!is_string($campaign)||strpos($campaign,'"levelCount": 20')===false||strpos($campaign,'"blight-king"')===false||!is_string($levels)||substr_count($levels,'"order":')<20||!is_string($manifest)||strpos($manifest,'"schemaVersion": 3')===false||strpos($manifest,'seed-man-approved-master-atlas-v1.webp')!==false||!is_string($index)||strpos($index,'20-Level Campaign')===false||strpos($index,'id="seed-man-level"')!==false||strpos($index,'world-mechanics-browser-v1.js')===false||!is_string($app)||strpos($app,'seed-man-base-runtime-v20')===false||strpos($app,'readEmbeddedLevel')!==false||!is_string($art)||strpos($art,'approved-showcase-2026-09-08')===false||!is_string($artCore)||strpos($artCore,'seed-man-approved-art-core-v4')===false||strpos($artCore,'seed-man-approved-master-atlas-v1.webp')!==false||!is_string($three)||strlen($three)<250000||strpos($three,'seed-man-three-public-v3')===false||strpos($three,'seed-man-three-dynamic-platforms-v1')===false||!is_string($runtime)||strpos($runtime,'seed-man-campaign-v20-runtime-v3')===false||!is_string($worldMechanics)||strpos($worldMechanics,'seed-man-world-mechanics-browser-v3')===false||strpos($worldMechanics,'seed-man-world-mechanics-runtime-v1')===false||!is_string($combat)||strpos($combat,'seed-man-combat-browser-v2')===false||strpos($combat,'syncBossState')===false||!is_string($enemyRuntime)||strpos($enemyRuntime,'seed-man-v20-enemy-runtime-v2')===false||!is_string($playerState)||strpos($playerState,'seed-man-player-state-v20')===false||!$validWebp($stage.'/assets/approved/seed-man-character-atlas-v2.webp')||!$validWebp($stage.'/assets/approved/seed-man-enemy-boss-atlas-v1.webp')||!$validWebp($stage.'/assets/approved/seed-man-platform-atlas-v1.webp')){$remove($stage);return new WP_Error('dtf_seed_contract','Staged v20 contract failed.',['status'=>409]);}
      ...
      return rest_ensure_response(['ok'=>true,'route'=>'/games/seed-man-platformer/','files'=>$written,'server_verified'=>true,'campaign_levels'=>20,'worlds'=>5,'final_boss'=>'blight-king','approved_art'=>true,'world_renderer'=>'seed-man-three-world-v2','world_mechanics'=>'seed-man-world-mechanics-browser-v3','dynamic_platforms'=>'seed-man-three-dynamic-platforms-v1','legacy_sprout_run'=>false,'corrupt_master_atlas'=>false,'player_state'=>'v20','combat_runtime'=>'v2','published_at'=>gmdate('c')]);
    }
  ]);
});
`.trim();
