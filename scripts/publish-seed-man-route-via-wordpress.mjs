...
const releaseMatch = indexText.match(/name="dtf-sprout-release" content="([^"]+)"/);
const release = releaseMatch?.[1] || '';
if (!/^\d{8}-r\d+$/.test(release)) throw new Error(`Invalid release marker: ${release}`);
for (const marker of [
  'Seed Man · Greenhouse Gauntlet',
  '0 / 24',
  'JUMP ×2',
  'stomp pests from above',
  `./gameplay-v2.js?v=${release}`,
  `./campaign-v1.js?v=${release}`
]) {
...
const files = releaseFiles.map((rel) => {
  ...
  if (!fs.existsSync(filePath)) throw new Error(`Missing release file: ${rel}`);
...
const namespace = `dtf-seed-man-publish/v2-${crypto.randomBytes(8).toString('hex')}`;
...
const purgeUrls = ['/games/seed-man-platformer/', ...releaseFiles.map((rel) => `/games/seed-man-platformer/${rel}`)];
...
async function queryPlugin() {
  ...
  return r.ok && Array.isArray(r.body)
    ? r.body.find((p) => String(p?.plugin || '').startsWith('code-snippets/')) || null
    : null;
}
...
async function waitForSnippetApi(safe = false) {
  ...
  for (let i = 1; i <= 12; i += 1) {
    try {
      const r = await wpRequest(`/wp-json/code-snippets/v1/snippets/schema${suffix}`, { allow: [404, 500] });
      if (r.ok) return true;
    } catch {}
...
add_action('rest_api_init', function () {
    ...
    register_rest_route($namespace, '/publish', [
        ...
        'permission_callback' => static function (WP_REST_Request $r) use ($token) {
            ...
            if ($supplied === '') $supplied = (string) $r->get_param('_dtf_seed_man_token');
            return $supplied !== '' && hash_equals($token, $supplied);
        },
        'callback' => static function (WP_REST_Request $r) use ($allowed, $purge_urls, $remove_tree, $release) {
            $body = $r->get_json_params();
            $incoming = isset($body['files']) && is_array($body['files']) ? $body['files'] : [];
            if (count($incoming) !== count($allowed)) {
                return new WP_Error('dtf_seed_file_count', 'Seed Man file count mismatch.', ['status' => 400, 'expected' => count($allowed), 'received' => count($incoming)]);
            }
            ...
            foreach ($incoming as $file) {
                ...
                if (!in_array($rel, $allowed, true) || isset($by[$rel])) {
                    return new WP_Error('dtf_seed_file_name', 'Unexpected or duplicate file.', ['status' => 400, 'rel' => $rel]);
                }
                $by[$rel] = $file;
            }
            ...
            $stage = wp_normalize_path(ABSPATH . 'games/.seed-man-stage-' . $nonce);
            $backup = wp_normalize_path(ABSPATH . 'games/.seed-man-backup-' . $nonce);
            ...
            $valid_markers = is_string($idx)
                && strpos($idx, 'Seed Man · Greenhouse Gauntlet') !== false
                && strpos($idx, '0 / 24') !== false
                && strpos($idx, 'stomp pests from above') !== false
                && strpos($idx, './gameplay-v2.js?v=' . $release) !== false
                && strpos($idx, './campaign-v1.js?v=' . $release) !== false
                && strpos($idx, 'name="dtf-sprout-release" content="' . $release . '"') !== false
                && strpos($idx, 'Original browser vertical slice') === false
                && strpos($idx, '0 / 8') === false;
            if (!$valid_markers) {
                $remove_tree($stage);
                return new WP_Error('dtf_seed_markers', 'Invalid staged markers.', ['status' => 409]);
            }
            ...
            $idx = @file_get_contents($target . '/index.html');
            $verified = $verified
                && is_string($idx)
                && strpos($idx, 'name="dtf-sprout-release" content="' . $release . '"') !== false
                && strpos($idx, './gameplay-v2.js?v=' . $release) !== false
            ...
            return rest_ensure_response([
                'ok' => true,
                'route' => '/games/seed-man-platformer/',
                'release' => $release,
                'files' => $written,
                'server_verified' => true,
                'cache_purge_fired' => true,
...
try {
  ...
  const created = await wpRequest('/wp-json/code-snippets/v1/snippets', {
    ...
    json: {
      ...
      desc: `Temporary authenticated publisher for Seed Man ${release}.`,
      code: snippetCode,
  ...
  if (result.body?.ok !== true || result.body?.server_verified !== true || result.body?.release !== release) {
    throw new Error(`Publish did not verify: ${JSON.stringify(result.body).slice(0, 1800)}`);
  }
