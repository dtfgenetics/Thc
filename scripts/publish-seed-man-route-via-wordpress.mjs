...
const releaseFiles=[
  '.htaccess','index.html','app-bootstrap-compat-v20.js','app.js','canvas-compat-v1.js','campaign-v1.js','campaign-v20-runtime.js','campaign-ui-v20.js','campaign-progression-v20.js',
  'approved-art-core-v1.js','approved-art-runtime-v1.js','seed-man-production-art.js','gameplay-v2.js',
  'v20-campaign-guard.js','v20-enemy-runtime.js','combat-browser-v2.js','enemy-attacks-browser-v2.js','enemy-attacks.js','three-world-v1.js',
...
for(const required of ['app-bootstrap-compat-v20.js','campaign-v20-runtime.js','v20-campaign-guard.js','campaign-ui-v20.js','campaign-progression-v20.js','v20-enemy-runtime.js','combat-browser-v2.js','enemy-attacks-browser-v2.js','approved-art-core-v1.js','approved-art-runtime-v1.js','seed-man-production-art.js'])if(!indexText.includes(required))throw new Error(`Seed Man public index missing v20 runtime: ${required}`);
for(const retiredIdentity of ['"id":"sprout-run"','bootstrap-speed','bootstrap-shield','bootstrap-magnet','bootstrap-jump'])if(indexText.includes(retiredIdentity))throw new Error(`Retired Seed Man bootstrap contract cannot be published: ${retiredIdentity}`);

const release=indexText.match(/name="dtf-sprout-release" content="([^"]+)"/)?.[1]||'20260909-v20';
...
add_action('rest_api_init', function () {
  ...
  register_rest_route($ns, '/publish', [
    ...
    'callback' => static function(WP_REST_Request $r) use ($token,$allowed,$purge,$remove) {
      ...
      $campaign=@file_get_contents($stage.'/data/campaign.json');$levels=@file_get_contents($stage.'/data/levels-20-v1.json');$art=@file_get_contents($stage.'/seed-man-production-art.js');$runtime=@file_get_contents($stage.'/campaign-v20-runtime.js');$progression=@file_get_contents($stage.'/campaign-progression-v20.js');$enemyRuntime=@file_get_contents($stage.'/v20-enemy-runtime.js');$combat=@file_get_contents($stage.'/combat-browser-v2.js');$attacks=@file_get_contents($stage.'/enemy-attacks-browser-v2.js');$index=@file_get_contents($stage.'/index.html');
      if(!is_string($campaign)||strpos($campaign,'"levelCount": 20')===false||strpos($campaign,'"blight-king"')===false||!is_string($levels)||substr_count($levels,'"order":')<20||!is_string($art)||strpos($art,'approved-showcase-2026-09-08')===false||!is_string($runtime)||strpos($runtime,'levelCount:20')===false||!is_string($progression)||strpos($progression,'seed-man-campaign-progression-v20')===false||strpos($progression,'selectLevel')===false||strpos($progression,'seedman:level-advanced')===false||!is_string($enemyRuntime)||strpos($enemyRuntime,'seed-man-v20-enemy-runtime-v2')===false||!is_string($combat)||strpos($combat,'seed-man-combat-browser-v2')===false||!is_string($attacks)||strpos($attacks,'seed-man-enemy-attacks-browser-v2')===false||!is_string($index)||strpos($index,'combat-browser-v1.js')!==false||strpos($index,'"id":"sprout-run"')!==false){$remove($stage);return new WP_Error('dtf_seed_contract','Staged v20 contract failed.',['status'=>409]);}
      ...
      return rest_ensure_response(['ok'=>true,'route'=>'/games/seed-man-platformer/','files'=>$written,'server_verified'=>true,'campaign_levels'=>20,'auto_advance'=>true,'progression_runtime'=>'seed-man-campaign-progression-v20','final_boss'=>'blight-king','approved_art'=>true,'combat_runtime'=>'seed-man-combat-browser-v2','published_at'=>gmdate('c')]);
    }
  ]);
});
`.trim();
