...
for (const rel of releaseFiles) {
  ...
  if (!fs.existsSync(filePath) || fs.statSync(filePath).size <= 0) throw new Error(`Missing release file: ${rel}`);
}
...
if (!releaseMatch) throw new Error('Seed Man release marker is missing from index.html.');
const release = releaseMatch[1];
if (!/^\d{8}-r\d+$/.test(release)) throw new Error(`Invalid Seed Man release marker: ${release}`);
...
for (const marker of [
  ...
  'JUMP ×2',
  release,
  'id="seed-man-campaign"',
  'sprout-run-campaign',
]) {
  if (!indexText.includes(marker)) throw new Error(`Missing canonical marker: ${marker}`);
}
for (const stale of ['Original browser vertical slice', '0 / 8', 'collect all eight sprouts']) {
  if (indexText.includes(stale)) throw new Error(`Retired marker remains: ${stale}`);
}

const campaignText = fs.readFileSync(path.join(sourceRoot, 'data/campaign.json'), 'utf8');
const campaign = JSON.parse(campaignText);
if (campaign?.id !== 'sprout-run-campaign' || campaign?.defaultLevelId !== 'sprout-run') {
  throw new Error('Campaign manifest does not preserve Sprout Run as the default level.');
}
if (!indexText.includes(`campaign-v1.js?v=${release}`)) throw new Error('Campaign runtime is not pinned to the current release.');
if (!indexText.includes(`app.js?v=${release}`)) throw new Error('Gameplay runtime is not pinned to the current release.');
...
if (process.env.SEED_MAN_PUBLISH_VALIDATE_ONLY === '1') {
  console.log(JSON.stringify({
    ok: true,
    mode: 'validate-only',
    release,
    files: files.map(({ rel, size, sha256 }) => ({ rel, size, sha256 })),
    campaign: campaign.id,
    defaultLevelId: campaign.defaultLevelId,
  }, null, 2));
  process.exit(0);
}
...
const allowedLiteral = JSON.stringify(releaseFiles);
...
async function wpRequest(route, { method = 'GET', json, headers = {}, allow = [] } = {}) {
  ...
  if (!response.ok && !allow.includes(response.status)) {
    throw new Error(`WordPress ${method} ${route} failed (${response.status}): ${typeof body === 'string' ? body.slice(0, 1200) : JSON.stringify(body).slice(0, 1200)}`);
  }
...
add_action('rest_api_init', function () {
    ...
    register_rest_route($namespace, '/publish', [
        ...
        'callback'=>static function(WP_REST_Request $r) use($allowed,$remove_tree,$release){
            ...
            if(!is_string($idx)||strpos($idx,'Seed Man · Greenhouse Gauntlet')===false||strpos($idx,'0 / 24')===false||strpos($idx,$release)===false||strpos($idx,'id="seed-man-campaign"')===false||strpos($idx,'Original browser vertical slice')!==false||strpos($idx,'0 / 8')!==false){$remove_tree($stage);return new WP_Error('dtf_seed_markers','Invalid staged markers.',['status'=>409]);}
            ...
            $idx=@file_get_contents($target.'/index.html');$verified=$verified&&is_string($idx)&&strpos($idx,'Seed Man · Greenhouse Gauntlet')!==false&&strpos($idx,'0 / 24')!==false&&strpos($idx,$release)!==false&&strpos($idx,'id="seed-man-campaign"')!==false&&strpos($idx,'Original browser vertical slice')===false&&strpos($idx,'0 / 8')===false;
            ...
            foreach($allowed as $rel)do_action('litespeed_purge_url','/games/seed-man-platformer/'.$rel);
            ...
            return rest_ensure_response(['ok'=>true,'route'=>'/games/seed-man-platformer/','release'=>$release,'files'=>$written,'server_verified'=>true,'cache_purge_fired'=>true,'published_at'=>gmdate('c')]);
        }
    ]);
});
`.trim();
...
try {
  ...
  const created = await wpRequest('/wp-json/code-snippets/v1/snippets', {
    method: 'POST',
    json: {
      name: `DTF Seed Man Atomic Publisher ${process.env.GITHUB_RUN_ID || Date.now()}`,
      desc: 'Temporary authenticated publisher for canonical Seed Man only.',
      code: snippetCode,
      tags: ['dtf-release', 'temporary', 'seed-man'],
      scope: 'global',
      priority: 1,
      active: false,
      network: false,
  ...
  if (!snippetId) throw new Error('Temporary publisher snippet missing ID.');
  ...
  const result = await wpRequest(`/wp-json/${namespace}/publish`, {
    ...
    json: { _dtf_seed_man_token: token, files },
  });
  if (result.body?.ok !== true || result.body?.server_verified !== true || result.body?.release !== release) {
    throw new Error(`Publish did not verify: ${JSON.stringify(result.body).slice(0, 1400)}`);
  }
