...
assert.match(css,/body:has\(> \.dtf-global-header\) \.sitebar,[\s\S]*body:has\(> \.dtf-global-header\) \.battle-top\{top:var\(--dtf-global-header-height,92px\)\}/,'desktop local sticky bars must clear the V5 header');
assert.match(css,/@media\(max-width:900px\)[\s\S]*body:has\(> \.dtf-global-header\) \.sitebar,body:has\(> \.dtf-global-header\) \.battle-top\{top:var\(--dtf-global-header-height,74px\)\}/,'tablet/mobile battle HUD must use the 74px V5 header contract');
assert.match(css,/body:has\(> \.dtf-global-header\) \.mobile-tabs\{top:calc\(var\(--dtf-global-header-height,74px\) \+ 64px\)\}/,'mobile board tabs must remain below the V5 header and battle HUD');
...
assert.match(sw,/ptp-shell-v9-burn-buds-v5-responsive-20260913/,'service-worker cache identity must refresh the responsive game-feel shell');
assert.match(sw,/\.\/gamefeel-v1\.css/,'game-feel layer must be available offline');
console.log('Burn Buds game-feel, V5 sticky-header, responsive containment and offline-shell contract passed.');
