...
assert.equal(data.prompts.length, 60, 'Bingo prompt pool should contain 60 prompts');
...
for (const mode of ['grow-room', 'bongwater']) {
  assert.equal(data.prompts.filter((prompt) => prompt.mode === mode).length, 30, `${mode} should have 30 prompts`);
}
