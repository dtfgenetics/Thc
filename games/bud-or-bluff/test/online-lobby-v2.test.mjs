...
const appSource = await fsPromises.then(fs => fs.readFile(path.join(publicDir, 'app-v2.js'), 'utf8'));
const htmlSource = await fsPromises.then(fs => fs.readFile(path.join(publicDir, 'index.html'), 'utf8'));
const cssSource = await fsPromises.then(fs => fs.readFile(path.join(publicDir, 'styles.css'), 'utf8'));
