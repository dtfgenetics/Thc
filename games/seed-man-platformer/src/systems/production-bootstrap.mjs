...
export async function bootstrapProductionGame({baseUrl='./',fetchImpl=globalThis.fetch}={}){
  ...
  const [manifest,levels,enemies,bosses,campaign,recipes,worlds,powerups]=await Promise.all([
    ...
    fetchJson(fetchImpl,`${baseUrl}data/authored-level-recipes-v1.json`),
    fetchJson(fetchImpl,`${baseUrl}data/world-gameplay-v1.json`),
    fetchJson(fetchImpl,`${baseUrl}data/powerup-catalog-v1.json`)
  ]);
