...
assert.deepStrictEqual(JSON.parse(publicLevelText), canonicalLevel, 'public legacy level JSON must match canonical compatibility level');
...
assert.ok(levelMatch, 'public page must embed a level payload');
...
// Production art is now strictly the approved character renderer. UI/game-shell ownership lives elsewhere.
...
assert.match(productionArt, /fallbackAllowed:false/, 'approved production art must not silently fall back to unapproved character art');
assert.match(productionArt, /window\.drawSeedManProduction=drawSeedManProduction/, 'approved production renderer must publish its render function');
assert.match(productionArt, /window\.drawSeedMan=drawSeedManProduction/, 'approved production renderer must initially own character rendering');
assert.match(productionArt, /character\.seedman\.atlas/, 'approved production renderer must consume the approved character atlas');
assert.match(productionArt, /activeApprovedPhenotype/, 'approved production art must preserve phenotype visuals');

assert.match(spriteRuntime, /approved-showcase-green-hero-v1/, 'sprite runtime v2 must preserve approved showcase identity');
assert.match(spriteRuntime, /2026-09-08-approved-showcase-sheet/, 'sprite runtime must expose approved reference marker');
assert.match(spriteRuntime, /window\.drawSeedMan=renderer/, 'approved runtime v2 must take final renderer ownership');
assert.match(spriteRuntime, /drawElementalVfx/, 'approved runtime must retain elemental effects');
assert.match(spriteRuntime, /drawPhenotypeLayer/, 'approved runtime must layer phenotype effects around movement poses');
