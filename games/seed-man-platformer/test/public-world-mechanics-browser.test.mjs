...
assert.match(source,/seed-man-world-mechanics-browser-v1/);
assert.match(source,/seed-man-world-mechanics-runtime-v1/);
for(const marker of ['moving-platforms','collapsing-platforms','conveyor-platforms','crystal-bounce','slippery-ground','wind-zones','heat-updraft'])assert.match(source,new RegExp(marker),`public mechanics runtime missing ${marker}`);
for(const phenotype of ['fire','electric','ice'])assert.match(source,new RegExp(`${phenotype}:`));
assert.match(html,/world-mechanics-browser-v1\.js\?v=20260915-world-mechanics-v1/,'world mechanics must be an explicit public dependency');
assert.ok(html.indexOf('app.js?v=20260909-v20-runtime-v5')<html.indexOf('world-mechanics-browser-v1.js'),'mechanics runtime must install after base physics');
assert.ok(html.indexOf('world-mechanics-browser-v1.js')<html.indexOf('campaign-v20-runtime.js'),'mechanics runtime must install before campaign selection');
assert.doesNotMatch(compat,/worldMechanicsDependencyLoader|document\.createElement\(['"]script['"]\)/,'runtime health bridge must remain metadata-only');
assert.match(entry,/three-world-dynamic\.mjs/,'public Three entry must use the dynamic-platform renderer wrapper');
assert.match(dynamicRenderer,/seed-man-three-dynamic-platforms-v1/);
assert.match(dynamicRenderer,/dynamicPlatformCount/);
new vm.Script(source,{filename:'world-mechanics-browser-v1.js'});
...
const fakeWindow={
  ...
  dispatchEvent(){},
  __SPROUT_COMBAT_BROWSER__:{snapshot:()=>({phenotypeForm:'fire',activePhenotype:'fire'})}
};
...
assert.equal(fakeWindow.__SEED_MAN_WORLD_MECHANICS__.version,'seed-man-world-mechanics-browser-v1');
...
assert.equal(dataset.seedManWorldMechanics,'seed-man-world-mechanics-browser-v1');

console.log(JSON.stringify({ok:true,version:fakeWindow.__SEED_MAN_WORLD_MECHANICS__.version,dynamicRenderer:'seed-man-three-dynamic-platforms-v1',dependency:'explicit'},null,2));
