...
test('creates a grow structure, diary entry, and persists after reload', async ({ page }) => {
  ...
  await spaceSection.getByLabel('Space name').fill('Test Tent');
  ...
  await cycleSection.getByLabel('Cycle name').fill('Foundation Cycle');
  await cycleSection.getByLabel('Grow space').selectOption({ label: 'Test Tent' });
  ...
  await plantSection.getByLabel('Plant name or ID').fill('BM-F3-01');
  await plantSection.getByLabel('Cultivar').fill('Blue Mango F3');
  await plantSection.getByLabel('Grow space').selectOption({ label: 'Test Tent' });
  await plantSection.getByLabel('Cycle').selectOption({ label: 'Foundation Cycle' });
