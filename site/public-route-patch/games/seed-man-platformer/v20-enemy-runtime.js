...
(() => {
  ...
  function buildEnemy(levelData, type, index, count, options = {}) {
    ...
    const start = options.carrier ? 0.62 : 0.12;
    const span = options.carrier ? 0 : 0.68;
    const fraction = options.carrier ? start : start + (index * span / Math.max(1,count - 1));
    ...
    const idSuffix = options.carrier ? `phenotype-${options.form}` : `${index + 1}-${type}`;
    const carrier = options.carrier ? PHENOTYPE_CARRIERS[options.form] : null;

    return {
      id:`v20-${levelData?.id || 'level'}-${idSuffix}`,
      archetype:type,
      ...
      phenotypeForm:options.form || null,
