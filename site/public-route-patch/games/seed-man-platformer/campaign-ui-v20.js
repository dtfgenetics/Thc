...
(() => {
  ...
  function currentPlayerName(){
    ...
    const name=sanitizePlayerName(input?.value||localStorage.getItem(PLAYER_NAME_KEY)||'');
  ...
  function installScoreboard(){
    const input=document.querySelector('#score-player-name');
    if(input){
      try{input.value=sanitizePlayerName(localStorage.getItem(PLAYER_NAME_KEY)||input.value);}catch{input.value=sanitizePlayerName(input.value);}
