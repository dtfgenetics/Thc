(() => {
  const ROOT_CODE_LENGTH = 6;
  const ROOT_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  const RUN_ROUNDS = 6;
  ...
  function normalizeRootCode(value) {
  ...
  function isValidRootCode(value) {
  ...
  function currentCase(state, data) {
    requireData(data);
    return getCase(data, state.current.caseId);
  }

  function inspect(inputState, inspectionId, data) {
    requireData(data);
  ...
  function diagnose(inputState, diagnosisId, data) {
    requireData(data);
    const state = clone(inputState);
  ...
  function advanceCase(inputState, data) {
    requireData(data);
    const state = clone(inputState);
  ...
  globalThis.RootCauseEngine = Object.freeze({
    ROOT_ALPHABET,
    ROOT_CODE_LENGTH,
    RUN_ROUNDS,
    MAX_INSPECTIONS,
